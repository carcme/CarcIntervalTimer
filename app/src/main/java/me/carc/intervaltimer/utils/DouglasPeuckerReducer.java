...
public class DouglasPeuckerReducer {
     ...
     */
    public static ArrayList<LatLon> reduceWithTolerance(ArrayList<LatLon> shape, double tolerance) {
        int n = shape.size();
        ...
        ArrayList<LatLon> newShape = new ArrayList<LatLon>(n); // the new shape to return
        for (int i = 0; i < n; i++) {
            if (marked[i])
     ...
     */
    private static void douglasPeuckerReduction(ArrayList<LatLon> shape, boolean[] marked, double tolerance, int firstIdx, int lastIdx) {
        ...
        LatLon firstPoint = shape.get(firstIdx);
        LatLon lastPoint = shape.get(lastIdx);

        for (int idx = firstIdx + 1; idx < lastIdx; idx++) {
            LatLon point = shape.get(idx);
     ...
     */
    public static double orthogonalDistance(LatLon point, LatLon lineStart, LatLon lineEnd) {
